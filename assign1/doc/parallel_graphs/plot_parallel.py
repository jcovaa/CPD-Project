...
def load_and_group(filename):
    ...
    df = pd.read_csv(path)
    return df.groupby(["Algorithm", "Size", "Threads"])[["Time_Sec", "GFlops"]].mean().reset_index()
...
if v1_data is not None and v2_data is not None:
    ...
    ax1.plot(v1_data["Size"], v1_data["Time_Sec"], color=COLORS["V1"], marker=MARKERS["V1"], label="V1: Column (ijk)")
    ax1.plot(v2_data["Size"], v2_data["Time_Sec"], color=COLORS["V2"], marker=MARKERS["V2"], label="V2: Line (ikj)")
    ...
    ax2.plot(v1_data["Size"], v1_data["GFlops"], color=COLORS["V1"], marker=MARKERS["V1"], label="V1: Column (ijk)")
    ax2.plot(v2_data["Size"], v2_data["GFlops"], color=COLORS["V2"], marker=MARKERS["V2"], label="V2: Line (ikj)")
...
if scale_data is not None:
    ...
    ax3.plot([4, 24], [1, 6], color=COLORS["Ideal"], linestyle="--", label="Ideal Scaling")
    ax3.plot(df_8192["Threads"], df_8192["Speedup"], color=COLORS["Speedup"], marker=MARKERS["Scale"], label="Measured Speedup")
    ...
    ax4.plot(df_8192["Threads"], df_8192["Efficiency"] * 100, color="#9b59b6", marker="h", label="Efficiency %")
